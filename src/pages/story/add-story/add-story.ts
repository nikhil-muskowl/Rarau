...
import { CanvasDrawPage } from "../../story/canvas-draw/canvas-draw";
...
export class AddStoryPage {
  ...
  constructor(public navCtrl: NavController,
    ...
    private storyService: StoryServiceProvider,
  ...
  takePicture() {
    ...
    if (this.segment === 'video') {
      ...
      this.cameraPreview.takePicture(pictureOpts).then(base64PictureData => {
        ...
        this.navCtrl.push(ShowPhotoPage, { photo: self.srcPhoto, story: true });
  ...
  share() {
    ...
    this.imageProvider.uploadFile(img).then((url) => {
      console.log('photo path', url);
      this.storyService.addStory({
        image: url,
        userName: this.user.name,
